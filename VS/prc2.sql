...
-- !!!!
CREATE OR REPLACE PROCEDURE proc_extend_loan (
    v_loan_id IN CHAR,
    v_book_id IN CHAR
)
IS
...
BEGIN
    -- Get current extension count and determine the due date
    SELECT 
        NVL(ExtensionCount, 0), 
        NVL(ExtendedDueDate, (SELECT DueDate FROM Loan WHERE LoanID = v_loan_id))
    INTO 
        v_extension_count, 
        v_current_due_date
    FROM LoanDetail
    ...
    -- Check if extension is within limit
    IF v_extension_count < 3 THEN
        UPDATE LoanDetail
        SET 
            ExtendedDueDate = v_current_due_date + 7, -- extend by 7 days
